...
class HistoricalDataCollector:
    ...
    def collect_comprehensive_data(self, symbol, timeframe='15m'):
        ...
        try:
            ...
            self.collect_perpetual_data(symbol, timeframe)
    ...
    def collect_spot_data(self, symbol, timeframe='15m'):
        ...
        try:
            # Collect spot OHLCV
            self.collect_spot_ohlcv(ccxt_symbol, timeframe)
            
            # Collect spot trades
            # self.collect_spot_trades(symbol)
            
        except Exception as e:
            logger.error(f"Failed to collect spot data for {symbol}: {e}")
    
    def collect_perpetual_data(self, symbol, timeframe='15m'):
        """Collect perpetual futures data (mark price, index price, funding rates, open interest, trades)"""
        ...
        try:
            ...
            self.collect_perpetual_mark_ohlcv(symbol, timeframe)
            
            # Collect perpetual index price OHLCV
            self.collect_perpetual_index_ohlcv(symbol, timeframe)
            
            # Collect funding rates
            self.collect_funding_rates(symbol)
            
            # Collect open interest
            self.collect_open_interest(symbol, timeframe)
            
            # Collect perpetual trades
            # self.collect_perpetual_trades(symbol)
            
        except Exception as e:
            logger.error(f"Failed to collect perpetual data for {symbol}: {e}")
    
    def collect_spot_ohlcv(self, symbol, timeframe='15m'):
    ...
    def collect_perpetual_mark_ohlcv(self, symbol, timeframe='15m'):
    ...
    def collect_perpetual_index_ohlcv(self, symbol, timeframe='15m'):
    ...
    def collect_funding_rates(self, symbol):
    ...
    def collect_open_interest(self, symbol, timeframe='5m'):
    ...
    def collect_perpetual_trades(self, symbol):
    ...
    def collect_historical_ohlcv(self, symbol, timeframe='15m', type=None):
        if type is None:
            raise ValueError("type is required")
            
        """Collect historical OHLCV data for multiple days"""

        logger.info(f"Collecting historical OHLCV data ({timeframe}) for {symbol}...")
    ...
    def collect_historical_ohlcv_index(self, symbol, timeframe='15m'):
        """Collect historical OHLCV index data for multiple days"""

        logger.info(f"Collecting historical OHLCV index data ({timeframe}) for {symbol}...")
    ...
    def collect_historical_funding_rates(self, symbol, type=None):
        if type is None:
            raise ValueError("type is required")
        ...
        logger.info(f"Collecting historical funding rates for {symbol}...")
        ...
        try:
            ...
            if funding_rates:
                ...
                # Save funding rates
                filename = f"{type}_{symbol}_funding_rates_{self.days}d.csv"
                df.to_csv(self.data_dir / filename, index=False)
    ...
    def collect_historical_open_interest(self, symbol, timeframe='5m', type=None):
        if type is None:
            raise ValueError("type is required")
        ...
        logger.info(f"Collecting historical open interest for {symbol}...")
        ...
        try:
            ...
            if all_open_interest:
                ...
                # Save open interest
                filename = f"{type}_{symbol}_open_interest_{self.days}d_{timeframe}.csv"
                df.to_csv(self.data_dir / filename, index=False)
    ...
    def collect_historical_trades(self, symbol, type=None):
        if type is None:
            raise ValueError("type is required")
            
        """Collect historical trades data (limited to recent period due to API limits)"""
        logger.info(f"Collecting historical trades for {symbol} (last {self.days} days)...")
        ...
        try:
            ...
            if all_trades:
                ...
                # Save trades
                filename = f"{type}_{symbol}_trades_{self.days}d.csv"
                df.to_csv(self.data_dir / filename, index=False)
